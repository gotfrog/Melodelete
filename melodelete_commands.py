...
class AutodeleteCommands(app_commands.Group):
    ...
    async def config(self, interaction: discord.Interaction,
                     ...
                     , messages: Optional[int], enablecrazysixrules:Optional[bool]
                     ) -> None:
        """View or set a channel's auto-delete settings"""
        if channel is None:
            channel = interaction.channel

        if hours is None and messages is None and enablecrazysixrules is None: # shows channel config
            ...
            self.config.set_channel(channel.id, time_threshold=time_threshold, max_messages=messages, usesCrazy6Rules=enablecrazysixrules)
